...
void Scene::rebuildModelInstances()
{
    modelInstances.clear();

    for (float x = -debug.width; x <= debug.width; x += 1.0f) {
        for (float y = -debug.width; y <= debug.width; y += 1.0f) {
            const glm::vec3 position(x * debug.spacing, 0.0f, y * debug.spacing);
            modelInstances.push_back(glm::translate(glm::mat4(1.0f), position));
        }
    }
}

void Scene::CasheInstanceData()
{
...
Scene::Scene()
{
    ...
    shadow = std::make_unique<ew::Shader>("assets/shaders/default_shadowmap_instanced.vs", "assets/shaders/toon_shadowmap.fs");
    ...
    //initialize the instanced buffer
    ...
    suzanne->bindInstanceBuffer(instanced_buffer);
}
...
void Scene::Render(void)
{
    ...
    //         //shadow->setMat4("model", modelInstances[static_cast<size_t>(i)]);
    //         suzanne->draw();
    //         ++i;
    //     }
    // }
...
void Scene::Debug(void)
{
    ...
    if (ImGui::SliderFloat("Spacing", &debug.spacing, 1.0f, 10.0f)) {
        CasheInstanceData();
    }
